	...
	char *cmd)
{
	...
	if (channel_cnt < 0) {
		ANDROID_ERROR(("Failed to get roamscan channels, err = %d\n", channel_cnt));
		return -1;
	}
...
int wl_android_reassoc(struct net_device *dev, char *command, int total_len)
{
	...
	if (total_len <
		...
		ANDROID_ERROR(("wl_android_reassoc: Invalid parameters \n"));
		return -1;
	}
	...
	(struct ether_addr *)&reassoc_params.bssid) == 0) {
		ANDROID_ERROR(("wl_android_reassoc: Invalid bssid \n"));
		return -1;
	}

	if (params->channel < 0) {
		ANDROID_ERROR(("wl_android_reassoc: Invalid Channel \n"));
		return -1;
	}
	...
	if (error) {
		ANDROID_ERROR(("wl_android_reassoc: failed to reassoc, error=%d\n", error));
		return -1;
	}
	return 0;
}
	...
	if (is_macaddr) {
		...
		if (ret) {
			...
			if (dtoh16(if_stats->version) > WL_IF_STATS_T_VERSION) {
				ANDROID_ERROR(("incorrect version of wl_if_stats_t,"
	...
	struct net_device *dev, char *command, int total_len)
{
	...
	ANDROID_ERROR(("%s ADPS Energy Gain: %d uAh\n", __FUNCTION__, energy_gain));
...
_wl_android_bcnrecv_start(struct bcm_cfg80211 *cfg, struct net_device *ndev, bool user_trigger)
{
	...
	if (unlikely(err)) {
		...
		ANDROID_INFO(("bcnrecv started. user_trigger:%d\n", user_trigger));
		if (user_trigger) {
...
wl_android_set_latency_crt_data(struct net_device *dev, int enable)
{
	...
	if (ret != BCME_OK) {
		ANDROID_ERROR(("failed to set latency_critical_data enable %d, error = %d\n",
			enable, ret));
		return ret;
	}
